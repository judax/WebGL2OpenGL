											...
											const ovrTracking * tracking, ovrMobile * ovr, ovrAppThread* appThread )
{
	...
	for ( int eye = 0; eye < VRAPI_FRAME_LAYER_EYE_MAX; eye++ )
	{
        ...
        (*(java->Env))->CallVoidMethod(java->Env, appThread->ActivityObject, appThread->setProjectionMatrixFromNativeMethodID, appThread->matrixFloatArray);
        (*(java->Env))->SetFloatArrayRegion(java->Env, appThread->matrixFloatArray, 0, 16, (jfloat*)&(eyeViewMatrix));
        (*(java->Env))->CallVoidMethod(java->Env, appThread->ActivityObject, appThread->setModelViewMatrixFromNativeMethodID, appThread->matrixFloatArray);
        
        // Call the java side so it can render a frame
        (*(java->Env))->CallVoidMethod(java->Env, appThread->ActivityObject, appThread->renderFrameFromNativeMethodID);
        // JUDAX END
...
static void ovrAppThread_Create( ovrAppThread * appThread, JNIEnv * env, jobject activityObject )
{
    ...
    appThread->activityObjectJClass = (*env)->GetObjectClass(env, appThread->ActivityObject);
    appThread->updateFromNativeMethodID = (*env)->GetMethodID(env, appThread->activityObjectJClass, "updateFromNative", "()V");
    appThread->renderFrameFromNativeMethodID = (*env)->GetMethodID(env, appThread->activityObjectJClass, "renderFrameFromNative", "()V");
    appThread->setProjectionMatrixFromNativeMethodID = (*env)->GetMethodID(env, appThread->activityObjectJClass, "setProjectionMatrixFromNative", "([F)V");
    appThread->setModelViewMatrixFromNativeMethodID = (*env)->GetMethodID(env, appThread->activityObjectJClass, "setModelViewMatrixFromNative", "([F)V");
    
    jfloatArray matrixFloatArray = (*env)->NewFloatArray(env, 16);
    appThread->matrixFloatArray = (jfloatArray)(*env)->NewGlobalRef(env, matrixFloatArray);
    // JUDAX END
